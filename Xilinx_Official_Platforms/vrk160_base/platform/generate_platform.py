...
platform = client.create_platform_component(name = platform_name, hw_design = xsa_path, emu_design = emu_xsa_path, no_boot_bsp = True, generate_dtb = False,
 desc = "The VRK160 Vitis base platform is built for the VRK160 evaluation kit. It provides a heterogeneous architecture with Arm® Cortex®-A72 application processors, Cortex®-R5F real-time processors, LPDDR5 memory, and RF support up to 18 GHz with ADC sample rates up to 32 GSPS. This platform enables development of high-performance applications in signal processing and compute-intensive domains using Versal™ adaptive SoCs." )
...
#linux domain
domain = platform.add_domain(os = "linux",cpu = "psv_cortexa72",name = "xrt",display_name = "xrt", generate_dtb = False)
